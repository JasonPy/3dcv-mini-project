...
Each dataset is split randomly into train and test according to the relative sizes in~\cite[Table 1]{shotton2013}. 

% WIP FALK:
Our data consists of a number of samples $\{(\boldsymbol{p}_i, \boldsymbol{w}_i)\}$ with
...
To validate the results, estimated camera poses are compared in terms of their translational and
angular error versus the true camera poses. The findings for each scene are subsequently compared
to the results of~\cite{shotton2013}. 
...
By applying the $L_2$-norm to the translation vectors, the translational error is obtained by
\begin{align}
...
\subsection{??}
Based on the metrics stated in the previous section, the amount of correctly estimated camera poses for each scene
is calculated. Therefore, the forest is evaluated with unseen images from the test data set and consecutive
RANSAC optimization predicts a camera pose for each image. These predictions are then compared to the 
ground truth camera poses and the amount of correctly classified poses recorded.

These findings are then compared to the results of~\cite{shotton2013}. 
