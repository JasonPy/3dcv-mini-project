...
In practice, a mean shift variance algorithm is used to find the center of the largest cluster.
...
is defined as
\begin{equation}
	f_{\theta}(\boldsymbol{p}) = I(\boldsymbol{p} + \frac{\boldsymbol{\delta}_1}{D(\boldsymbol{p})}, c_1)
	- I(\boldsymbol{p} + \frac{\boldsymbol{\delta}_2}{D(\boldsymbol{p})}, c_2) \geq \tau
\end{equation}
where $I(\boldsymbol{p}, c)$ is the images value in the $c \in \{\text{r, g, b}\}$ component at
the pixel location $\boldsymbol{p}$ with the parameters given explicitly as
$\theta = \{\boldsymbol{\delta}_{1,2}, c_{1,2}, \tau\}$. The offsets $\boldsymbol{\delta}_{1,2}$
...
disregarded in training and can also not be predicted by the forest.
