...
class DataLoader:
    ...
    def load_dataset(self, scene_name: str, image_indices: np.array):
        ...
        for file_path in tqdm(files_to_load, ascii = True, desc = f'Loading image data', dynamic_ncols = True):
    ...
    def sample_from_data_set(self, images_data: Tuple[np.array, np.array, np.array], num_samples: int) -> Tuple[np.array, np.array]:
        ...
        for i in tqdm(range(num_images), ascii = True, desc = 'Generating samples', dynamic_ncols = True):
